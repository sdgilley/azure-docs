...
* Grant the managed identity for the compute cluster access to the secrets stored in key vault.  Or, for serverless compute, grant the managed identity used to submit jobs access to the secrets. The method used to grant access depends on how your key vault is configured:
